# nn_disruptions_pipeline.py
# Keras MLP pipeline a 'disruptions-2024.csv' fájlra.
# Használat:
#  - python nn_disruptions_pipeline.py
...
# ====== CONFIG ======
DATA_PATH = '../Data/disruptions-2024.csv'   # Állítsd ide a fájl valódi elérési útját (pl. r"C:\... \disruptions-2024.csv")
OUTPUT_XLSX = 'Predicted_values_nn.xlsx'
USE_LOG_TARGET = False   # Ha True: log1p(target)-on tanítunk, visszatranszformálunk expm1-el
CLIP_OUTLIERS = False    # Ha True: clip-eljük a targetet felső határral (CLIP_MAX)
CLIP_MAX = 2000.0
TEST_FRACTION = 0.2
RANDOM_SEED = 1
EPOCHS = 200
...
# egyszerű kategória enkódolások, csak ha léteznek
...
# opcionális clipping
if CLIP_OUTLIERS:
    df['duration_minutes'] = df['duration_minutes'].clip(upper=CLIP_MAX)

# log-transzformáció opció
if USE_LOG_TARGET:
    df['target'] = np.log1p(df['duration_minutes'])
else:
    df['target'] = df['duration_minutes']

# ----- features list (kiterjeszthető)
features = [
...
# ----- train/test split időrendben (idő szerint rendezetten)
df_sorted = df.sort_values(by=time_col).reset_index(drop=True)
X_sorted = pd.DataFrame(scaler.transform(df_sorted[features].fillna(-1)), columns=features)
y_sorted = df_sorted['target'].reset_index(drop=True)
...
print("USE_LOG_TARGET:", USE_LOG_TARGET, " CLIP_OUTLIERS:", CLIP_OUTLIERS)

# ----- Keras modell (egyszerű MLP)
...
callbacks = []
if EARLY_STOPPING:
    callbacks.append(EarlyStopping(monitor='val_loss', patience=PATIENCE, restore_best_weights=True, verbose=1))
...
# ----- predict és visszatranszformáció ha szükséges
...
# ----- metrikák (valós skálán)
r2 = r2_score(y_test_raw, y_pred_raw)
mae = mean_absolute_error(y_test_raw, y_pred_raw)
mse = mean_squared_error(y_test_raw, y_pred_raw)
rmse = np.sqrt(mse)
median_abs = median_absolute_error(y_test_raw, y_pred_raw)
sm = smape(y_test_raw, y_pred_raw)
...
N = min(300, len(y_test_raw))
plt.figure(figsize=(14,6))
plt.plot(range(N), y_pred_raw[:N], label='Predicted', linestyle='-')
plt.plot(range(N), y_test_raw[:N], label='Actual', linestyle='-')
plt.xlabel('Test sample index (first N)')
plt.ylabel('Duration (minutes)')
plt.title('NN Prediction (first N samples)')
plt.legend()
plt.grid(True)
plt.show()
...
out_df['predicted_minutes'] = y_pred_raw
